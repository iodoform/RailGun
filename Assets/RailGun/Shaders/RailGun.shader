...
Shader "Custom/RailGun"
{
    ...
    SubShader
    {
        ...
        void surf (Input IN, inout SurfaceOutput o)
        {
            ...
            if (pow(convertedPos.x-convertedShotPoint.x,2.0)+pow(convertedPos.y-convertedShotPoint.y,2)<=pow(_Radius,2.0)) o.Alpha = 0;
        ...
        float distance(float3 pos)
        {
            ...
            float tmp = _ShotDirection.z/length(_ShotDirection.xyz);
            float theta = acos(tmp);
            float3 axis = length(cross(_ShotDirection,float3(0,0,1)))==0 ? float3(0,0,1) : normalize(cross(_ShotDirection,float3(0,0,1)));
            float3 convertedShotPoint = rotateQuaternion(theta,axis,_ShotPoint.xyz);
            float3 convertedPos = rotateQuaternion(theta,axis,pos);
            return sqrt(pow(convertedPos.x-convertedShotPoint.x,2)+pow(convertedPos.y-convertedShotPoint.y,2))-_Radius;
        }
        void surf (Input IN, inout SurfaceOutputStandard o)
        {
            ...
            {
                o.Emission = _EmissionColor*(1/(1+distance(IN.worldPos)*_AttenuationRate))*_Strength*o.Alpha;
                if(pow(convertedPos.x-convertedShotPoint.x,2)+pow(convertedPos.y-convertedShotPoint.y,2)<=pow(_Radius,2))o.Alpha = 0;
            }
